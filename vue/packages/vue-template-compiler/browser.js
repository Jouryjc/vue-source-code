...
/*  */
/**
 * @file 标记static静态节点，后面当update更新界面时，会有一个patch的过程，diff算法会直接跳过静态节点，从而减少了比较的过程，优化了patch的性能。
 */
...
function genHandler (
  ...
  var isMethodPath = simplePathRE.test(handler.value);     // 匹配是不是方法路径
  var isFunctionExpression = fnExpRE.test(handler.value);  // 匹配是不是函数
